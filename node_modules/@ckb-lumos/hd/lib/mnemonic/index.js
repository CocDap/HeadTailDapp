...
exports.generateMnemonic = exports.validateMnemonic = exports.entropyToMnemonic = exports.mnemonicToEntropy = exports.mnemonicToSeed = exports.mnemonicToSeedSync = void 0;
const crypto_1 = __importDefault(require("crypto"));
const word_list_1 = __importDefault(require("./word_list"));
const RADIX = 2048;
...
if (word_list_1.default.length !== RADIX) {
    throw new Error(`Word list should have ${RADIX} words, but ${word_list_1.default.length} received in fact`);
}
...
function deriveChecksumBits(entropyBuffer) {
    ...
    const CS = ENT / 32;
    const hash = crypto_1.default.createHash("sha256").update(entropyBuffer).digest();
...
function mnemonicToSeedSync(mnemonic = "", password = "") {
    ...
    return crypto_1.default.pbkdf2Sync(mnemonicBuffer, saltBuffer, PBKDF2_ROUNDS, KEY_LEN, "sha512");
}
...
function mnemonicToSeed(mnemonic = "", password = "") {
    return new Promise((resolve, reject) => {
        try {
            const mnemonicBuffer = Buffer.from(mnemonic.normalize("NFKD"), "utf8");
            const saltBuffer = Buffer.from(salt(password.normalize("NFKD")), "utf8");
            crypto_1.default.pbkdf2(mnemonicBuffer, saltBuffer, PBKDF2_ROUNDS, KEY_LEN, "sha512", (err, data) => {
                if (err) {
                    reject(err);
                }
                resolve(data);
...
function mnemonicToEntropy(mnemonic = "") {
    ...
    const bits = words
        .map((word) => {
        const index = word_list_1.default.indexOf(word);
        if (index === -1) {
            throw new Error(INVALID_MNEMONIC);
        }
...
function entropyToMnemonic(entropyStr) {
    ...
    const words = chunks.map((binary) => {
        ...
        return word_list_1.default[index];
    });
    return words.join(" ");
}
...
function generateMnemonic() {
    ...
    const entropy = "0x" + crypto_1.default.randomBytes(entropySize).toString("hex");
